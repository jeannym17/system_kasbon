...
class PertanggunganAtasanUser2Controller extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $title = 'Pertanggungan';
        return view('vkp-atasan-user-2.index', compact('pertanggungan', 'title'))->with('i', (request()->input('page', 1) - 1) * 5);
    }
    ...
    public function cek_pertanggungan($id)
    {
        ...
        return view('vkp-atasan-user-2.cek_pertanggungan', compact('title', 'pertanggungan'));
    }
    ...
    public function cek_pertanggungan_edit($id)
    {
        ...
        return view('vkp-atasan-user-2.cek_pertanggungan_edit', compact('title', 'pertanggungan', 'detail'));
    }
    ...
    public function show_v($id)
    {
        $pertanggungan = pertanggungan::find($id);
        $title = 'Pertanggungan';
        return view('vkp-atasan-user-2.lihat-kasbon-v', compact('pertanggungan', 'title'));
    }
    ...
    public function kelengkapan_v($id)
    {
        $pertanggungan = pertanggungan::find($id);
        $title = 'Pertanggungan';
        return view('vkp-atasan-user-2.lihat-kelengkapan-v', compact('pertanggungan', 'title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($pertanggungan, $request, $id, $now) {
            ...
            if ($pertanggungan->verifikasipertanggungan->vkp_a_12 = $request->Input('status') == 'Terverifikasi') {
            ...
            } elseif ($pertanggungan->verifikasipertanggungan->vkp_a_12 = $request->Input('status') == 'Ditolak') {
                $pertanggungan->verifikasipertanggungan->vkp_a_12 = $request->Input('status');
            ...
            $pertanggungan->verifikasipertanggungan->tgl_vkp_a_12 = $now;
            $pertanggungan->verifikasipertanggungan->updated_at = $now;
            $pertanggungan->verifikasipertanggungan->id_vkp_a_12 = Auth::user()->id;
            $pertanggungan->verifikasipertanggungan->save();
        });

        return redirect()->route('vkp-atasan-user-2.index')->with('success', 'Pertanggungan updated successfully');
    }
    ...
    public function update(Request $request, $id)
    {
        DB::transaction(function () use ($request, $id) {
            ...
            $pertanggungan->verifikasipertanggungan->update([
                'vkp_a_12' => $request->Input('status'),
                'id_vkp_a_12' => Auth::user()->id,
                'tgl_vkp_a_12' => $now
            ]);
        ...
        return redirect()->route('vkp-atasan-user-2.index')
