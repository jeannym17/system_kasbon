...
class NonKasbonAtasanUser2Controller extends Controller
{
    ...
    public function index()
    {
        ...
        $title = 'nonkasbon';
        return view('vnk-atasan-user-2.index', compact('nonkasbon', 'title'))->with('i', (request()->input('page', 1) - 1) * 5);
    }
    ...
    public function show($id)
    {
        ...
        $title = 'Non Kasbon';
        return view('vnk-atasan-user-2.lihat-nonkasbon', compact('nonkasbon', 'title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($nonkasbon, $request, $id) {
            ...
            if ($nonkasbon->verifikasinonkasbon->vnk_a_12 = $request->Input('status') == 'Terverifikasi') {
                $nonkasbon->verifikasinonkasbon->vnk_a_12 = $request->Input('status');
                $nonkasbon->verifikasinonkasbon->vnk = 'Dalam Proses';
            } elseif ($nonkasbon->verifikasinonkasbon->vnk_a_12 = $request->Input('status') == 'Ditolak') {
                $nonkasbon->verifikasinonkasbon->vnk_a_12 = $request->Input('status');
            ...
            $nonkasbon->verifikasinonkasbon->tgl_vnk_a_12 = $now;
            $nonkasbon->verifikasinonkasbon->id_vnk_a_12 = Auth::user()->id;
            $nonkasbon->verifikasinonkasbon->updated_at = $now;
            $nonkasbon->verifikasinonkasbon->save();
        });

        return redirect()->route('vnk-atasan-user-2.index')
    ...
    public function update(Request $request, $id)
    {
        DB::transaction(function () use ($request, $id) {
            ...
            $nonkasbon->verifikasinonkasbon->update([
                'vnk_a_12' => $request->Input('status'),
                'tgl_vnk_a_12' => $now,
                'id_vnk_a_12' => Auth::user()->id,
                'updated_at' =>  $now
            ]);
        ...
        return redirect()->route('vnk-atasan-user-2.index')->with('success', 'Non Kasbon updated successfully');
    }
}
