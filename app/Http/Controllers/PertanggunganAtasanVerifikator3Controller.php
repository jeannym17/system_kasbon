...
class PertanggunganAtasanVerifikator3Controller extends Controller
{
    ...
    public function index()
    {
        ...
        return view('vkp-atasan-verifikator-3.index', compact('pertanggungan', 'title'))
    ...
    public function cek_pertanggungan($id)
    {
        ...
        $title = 'Detail';
        return view('vkp-atasan-verifikator-3.cek_pertanggungan', compact('title', 'pertanggungan'));
    }
    public function cek_pertanggungan_edit($id)
    {
        ...
        $title = 'Detail';
        return view('vkp-atasan-verifikator-3.cek_pertanggungan_edit', compact('title', 'pertanggungan', 'detail'));
    }
    public function show_v($id)
    {
        $pertanggungan = pertanggungan::find($id);
        $title = 'Pertanggungan';
        return view('vkp-atasan-verifikator-3.lihat-kasbon-v', compact('pertanggungan', 'title'));
    }
    ...
    public function kelengkapan_v($id)
    {
        $pertanggungan = pertanggungan::find($id);
        $title = 'Pertanggungan';
        return view('vkp-atasan-verifikator-3.lihat-kelengkapan-v', compact('pertanggungan', 'title'));
    }
    ...
    public function update(Request $request, $id)
    {
        DB::transaction(function () use ($request, $id) {
            ...
            $now = Carbon::now('Asia/Jakarta');
            $pertanggungan->verifikasipertanggungan->tgl_vkp_a_4 = $now;
            $pertanggungan->verifikasipertanggungan->updated_at = $now;
            $pertanggungan->verifikasipertanggungan->id_vkp_a_4 = Auth::user()->id;
            $pertanggungan->verifikasipertanggungan->save();
        });
        return redirect()->route('vkp-atasan-verifikator-3.index')->with('success', 'Pertanggungan updated successfully');
    }
}
