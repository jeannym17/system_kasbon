...
class PertanggunganAtasanVerifikator1Controller extends Controller
{
    ...
    public function index()
    {
        ...
        return view('vkp-atasan-verifikator-1.index', compact('pertanggungan', 'title'))
    ...
    public function cek_pertanggungan($id)
    {
        ...
        $title = 'Detail';
        return view('vkp-atasan-verifikator-1.cek_pertanggungan', compact('title', 'pertanggungan'));
    }
    public function cek_pertanggungan_edit($id)
    {
        ...
        $title = 'Detail';
        return view('vkp-atasan-verifikator-1.cek_pertanggungan_edit', compact('title', 'pertanggungan', 'detail'));
    }
    ...
    public function show_v($id)
    {
        $pertanggungan = pertanggungan::find($id);
        $title = 'Pertanggungan';
        return view('vkp-atasan-verifikator-1.lihat-kasbon-v', compact('pertanggungan', 'title'));
    }
    ...
    public function kelengkapan_v($id)
    {
        $pertanggungan = pertanggungan::find($id);
        $title = 'Pertanggungan';
        return view('vkp-atasan-verifikator-1.lihat-kelengkapan-v', compact('pertanggungan', 'title'));
    }
    ...
    public function update(Request $request, $id)
    {
        DB::transaction(function () use ($request, $id) {
            ...
            if ($pertanggungan->kasbon->k_total > 0 && $pertanggungan->kasbon->k_total <= 20000000) {
            ...
            } elseif ($pertanggungan->kasbon->k_total > 20000000) {
                if ($pertanggungan->verifikasipertanggungan->vkp_a_2 = $request->Input('status') == 'Terverifikasi') {
                    $pertanggungan->verifikasipertanggungan->vkp_a_2 = $request->Input('status');
            ...
            $now = Carbon::now('Asia/Jakarta');
            $pertanggungan->verifikasipertanggungan->tgl_vkp_a_2 = $now;
        ...
        return redirect()->route('vkp-atasan-verifikator-1.index')->with('success', 'Pertanggungan updated successfully');
    }
}
